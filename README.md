...
通过 Navicat 或其他 MySQL 客户端执行 `sql/init.sql`，会自动创建数据库 `cms_competition`、全部表结构及测试数据：
...
│           ├── auth/                     # 登录、注册
│           ├── public/                   # 首页、竞赛列表、竞赛详情
│           ├── student/                  # 个人中心、报名、团队管理（11 页）
│           ├── teacher/                  # 竞赛管理、审核、统计（4 页）
│           └── admin/                    # 竞赛、报名、轮播图、统计、用户（6 页）
├── sql/                                  # 数据库脚本
│   └── init.sql                          # 主初始化脚本
└── README.md
```
...
**登录报 Unknown column 'college'？** 通常是 `init.sql` 中列名 `school` 与实体类 `User.college` 不一致导致。MyBatis-Plus 按 camelCase→snake_case 映射，实际查询的是 `college` 列。确认 `init.sql` 中 `user` 表列名为 `college`（非 `school`），或执行 `ALTER TABLE user CHANGE COLUMN school college VARCHAR(100);` 修复。
